...
<RevealBox title="Code: Blogpost-Laden und Slug-Verarbeitung">
```ts
...
export const listBlogPosts = async (): Promise< Omit<BlogPostData, 'component'>[] > => {
...
<RevealBox title="Tipp: Metadaten für SEO und Social Media">
Die Metadaten werden automatisch für Title, Description, OpenGraph und Twitter Cards verwendet. So sieht jeder Blogpost beim Teilen auf Social Media optimal aus!
</RevealBox>
...
<RevealBox title="Code: Unterschied Markdown vs. MDX">
```md
<!-- Markdown (.md): -->
# Mein Blogpost
...
<RevealBox title="Technisches Detail">
  Hier kann ich beliebige React-Komponenten einbinden!
</RevealBox>
```
</RevealBox>
...
<RevealBox title="Code: Automatisches Komponenten-Mapping in MDX">
```tsx
// mdx-components.tsx
...
## 🚩 Learnings & Tipps für eigene Projekte

- **MDX statt Markdown:** Für moderne, interaktive Blogposts ist MDX die beste Wahl.
- **Klare Struktur:** Ein sauberer Ordneraufbau und konsequente Komponenten-Trennung erleichtern Wartung und Erweiterung.
- **Metadaten nutzen:** Gute Metadaten sind Gold wert für SEO, Social Media und Navigation.
- **Automatisierung:** Dynamisches Routing und automatisches Komponenten-Mapping sparen viel Zeit.
- **Fehlerquellen:** Achtung bei Slugs, Imports und Build-Fehlern – gutes Error-Handling ist Pflicht!

---

## 🔜 Wie geht’s weiter?

Im nächsten Teil zeige ich, wie ich die **Blogübersicht** gestaltet habe – mit Tag-Filter, Suche und ansprechender Vorschau für alle Beiträge. Außerdem gehe ich auf die Filter- und Suchlogik sowie die Performance-Optimierung ein.
